...
const resolvedOutPng = args.outPng.startsWith("/workspace") && !args.outPng.startsWith(process.cwd())
  ? args.outPng.replace(/^\/workspace/, process.cwd())
  : args.outPng;
const outPng = checkedOutputPath(resolvedOutPng, allowedDirs);
...
try {
  ...
  for (const vp of VIEWPORTS) {
    ...
    await page.screenshot({ path: vp.screenshot, fullPage: false, timeout: 10000 });
    await page.close();
